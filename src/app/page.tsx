    ...
    <main className="bg-transparent">
      ...
      <section className="flex flex-col items-center justify-center text-center py-32 relative bg-transparent">
        <h1 className="text-5xl md:text-7xl font-plak font-extrabold leading-tight">
          ...
          <div className="text-orange underline decoration-orange decoration-2 underline-offset-4">Edwance</div>
          <div className="text-neutral-black">learning is here.</div>
        </h1>
        <div className="mt-8 flex gap-4 justify-center">
          <button className="bg-orange text-white px-8 py-4 rounded-xl font-semibold text-lg shadow-orange hover:bg-orangeDark transition">Try Now</button>
          <button className="border-2 border-orange text-orange px-8 py-4 rounded-xl font-semibold text-lg hover:bg-orange hover:text-white transition">Book a Demo</button>
        </div>
      </section>
      ...
      <section className="relative min-h-[80vh] flex flex-col items-center justify-center bg-gradient-to-br from-orange to-orangeLight py-24 overflow-hidden">
        ...
        <div className="flex items-center justify-center gap-8 mb-8">
          <button onClick={() => setCarouselIndex((i) => (i - 1 + slides.length) % slides.length)} className="text-white text-3xl px-4">&#8592;</button>
          <AnimatePresence mode="wait">
            ...
            >
              <div className="flex-1 text-left text-white max-w-lg">
                <h3 className="text-6xl md:text-8xl font-plak font-extrabold mb-6">{slides[carouselIndex].title}</h3>
                <p className="text-lg font-poppins font-light leading-relaxed">{slides[carouselIndex].text}</p>
              </div>
              <div className="flex-1 flex justify-center">
                <img src={slides[carouselIndex].image} alt="slide" className="w-96 h-80 object-contain rounded-2xl bg-white/30 p-8 shadow-2xl" />
              </div>
            </motion.div>
          </AnimatePresence>
          <button onClick={() => setCarouselIndex((i) => (i + 1) % slides.length)} className="text-white text-3xl px-4">&#8594;</button>
        </div>
        <div className="flex gap-2 justify-center mt-4">
          {slides.map((_, i) => (
            <span key={i} className={`w-3 h-3 rounded-full ${i === carouselIndex ? "bg-white" : "bg-white/50"}`}></span>
          ))}
        </div>
      ...
      {/* Contact Section */}
      <section className="relative min-h-[80vh] flex flex-col items-center justify-center py-24 bg-transparent overflow-hidden">
      ...
      <section className="relative min-h-[80vh] flex flex-col items-center justify-center bg-gradient-to-br from-orange to-orangeLight py-24 overflow-hidden">
        ...
        <div className="flex items-center justify-center gap-8">
          <button onClick={() => setTeamIndex((i) => (i - 1 + team.length) % team.length)} className="text-white text-3xl px-4">&#8592;</button>
          <AnimatePresence mode="wait">
            <motion.div
              key={teamIndex}
              initial={{ opacity: 0, x: 100 }}
            ...
            >
              ...
              <div className="text-2xl font-plak font-bold text-white mb-2">{team[teamIndex].name}</div>
              <div className="text-lg text-white/80 mb-1">{team[teamIndex].role}</div>
              <div className="text-base text-white/70">{team[teamIndex].experience}</div>
            </motion.div>
          </AnimatePresence>
          <button onClick={() => setTeamIndex((i) => (i + 1) % team.length)} className="text-white text-3xl px-4">&#8594;</button>
        </div>
      </section>
    </main>
  );
